    ...
    return View(model); // Return the view with validation errors if any
        ...
        public async Task<IActionResult> LoginAdmin(AdminLoginViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                {
                    // Use BCrypt to verify the password
                    bool isPasswordValid = BCrypt.Net.BCrypt.Verify(model.PasswordHash, Admin.PasswordHash);
                    ...
                    if (isPasswordValid)
                    {
                        ...
                        var identity = new ClaimsIdentity(claims, "LoginAdmin");
                        var principal = new ClaimsPrincipal(identity);

                        // Perform sign-in
                        await HttpContext.SignInAsync(principal);

                        return RedirectToAction("Index", "Admin");  // Redirect to home or any other page after login
                    }
                    else
                    {
                        ModelState.AddModelError("", "Invalid password.");
                ...
                {
                    ModelState.AddModelError("", "No Admin found with this email.");
                }
            }
